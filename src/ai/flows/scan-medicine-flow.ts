 ...
 *
 * - scanMedicine - A function that handles the medicine identification process.
 * - ScanMedicineInput - The input type for the scanMedicine function.
 * - ScanMedicineOutput - The return type for the scanMedicine function.
 */

import {ai} from '@/ai/genkit';
import {z} from 'genkit';

const ScanMedicineInputSchema = z.object({
  imageDataUri: z.string().describe("A data URI of an image of a medicine (pill, box, or strip). Expected format: 'data:<mimetype>;base64,<encoded_data>'."),
});
export type ScanMedicineInput = z.infer<typeof ScanMedicineInputSchema>;

const ScanMedicineOutputSchema = z.object({
...
export type ScanMedicineOutput = z.infer<typeof ScanMedicineOutputSchema>;


export async function scanMedicine(input: ScanMedicineInput): Promise<ScanMedicineOutput> {
  return scanMedicineFlow(input);
}

const prompt = ai.definePrompt({
  name: 'scanMedicinePrompt',
  input: {schema: ScanMedicineInputSchema},
  output: {schema: ScanMedicineOutputSchema},
  prompt: `You are a professional medical AI assistant specialized in medicine identification and safety.

Your job is to analyze the medicine from the provided image (which could be a pill, a box, or a strip), identify the medicine name from any text visible on it (OCR), and provide accurate, safe, and structured information. If text is not clearly visible, try to identify the medicine based on its appearance.

Image of the medicine: {{media url=imageDataUri}}
...
const scanMedicineFlow = ai.defineFlow(
  {
    name: 'scanMedicineFlow',
    inputSchema: ScanMedicineInputSchema,
    outputSchema: ScanMedicineOutputSchema,
  },
  async input => {
